...
```{r}
five_or_more <- filter(bird_type, n >= 5)
less_five <- filter(bird_type, n<5)
```
...
ggplot(bird_type, aes(egg_mass_median, male_mass_median)) +
  geom_point() +
  geom_point(color = "red", data = five_or_more, size = 3, alpha = 0.8) +
  geom_text(aes(y=male_mass_median * 0.6, label = type), data = five_or_more) +
  scale_x_log10() +
  scale_y_log10() +
  annotate("text", x = 3, y = 1000, label = "The smaller set of birds is in red") +
  xlab("Median egg mass") +
  ylab("median_male_mass")
```
