...
## Methods
To obtain this dataset, I went online and found a 200 person sample. I then checked for errors and created a CSV with the data. The models used here are ways to tell how two or more means are related.Then the model describes the best fit line for the data.
...
```
The graphic here shows that the best fit line is just below a slope of 1, meaning that males tended to overestimate their height
```{r}
...
In hypothesis 2, the same problem arose. The slope here was roughly a 1, representing actual height, and was considered significant at a 95% confidence level. But the intercept, representing reported height, was not significant, which means that there is not significant data that proves men will overstate their height. This large range in reported height shows that there is not much of a trend going in one way or another when it comes to men overstating or understating their height. 

In hypothesis 3, the numbers were different. At a 95% confidence level, both the slope (actual weight) and the intercept (reported weight) were significant. This means that womens' reported weight was higher than what it actually was, which disproves this hypothesis. Even though the slope is less than 1, the intercept is never less than 0 which means that reported weight will always be higher than actualy weight in this model. 
