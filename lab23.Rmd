...
**Answer**: 17% of the total crimes ended in an arrest
...
**Answer**: 27.5% of total crimes were thefts
...
**Answer**: 38816 of the 141,064 crimes are thefts
...
**Answer**:36.5% of households had a theft
...
**Answer**: It is larger than the mean of the entire dataset. It is a smaller area which means that the data is more concentrated
...
**Answer**: It is smaller than the total dataset mean because there is less data to work with, and there can be areas that have more thefts than others.
...
```{r}
houseca <- filter(ca, area_number == "23")
sum(houseca$num_households)
```
...
```{r}
ca2$theft_rate <- (ca2$theft_sum/sum(ca2$num_households)) * 1000
```
...
```{r}
ggplot(ca2, aes(median_income, theft_rate)) +
geom_point()
```

Describe the general pattern and any outliers.

**Answer**: Most of the theft rates are below a 1
...
**Answer**: Near North Side or Loop
...
**Answer**: Wealthy people built mansions in the area years ago and now its possible those people moved out, causing a wealth disarity and pushing crime to a higher level than other areas of chicago.
