...
Answer: The larger the male mass is the larger the egg mass is. 
...
ggplot(birds, aes (male_mass, egg_mass)) +
  ...
  annotate("text", x = 35, y = 50, label = "red points are parrots" ) +
  scale_x_log10() +
  scale_y_log10()
```
...
Answer:Yes the best fit line follows the trend between the size of a bird and the size of its eggs. 
...
```{r}
tropic <- filter(birds, type =="Tropicbird")
ggplot(birds, aes (male_mass, egg_mass)) +
  geom_point(size = .15) +
  geom_point(color="red", data = tropic)+
  annotate("text", x = 35, y = 50, label = "The red point is the red-tailed tropicbird" ) +
  scale_x_log10() +
  scale_y_log10()
```
...
```{r}
large <- filter(birds, male_mass > 6000)
small <- filter(birds, male_mass < 100)
ggplot(birds, aes (mating_system, egg_mass)) +
  geom_point() +
  geom_point(color = "red", data = large) +
  geom_point(color = "blue", data = small) +
  annotate("text", x= 3.5, y= 400, label = "The red points show eggs with male birds larger than 6000 grams")+
  annotate("text", x = 3.5, y=300, label = "The blue points show eggs with male birds smaller than 100 grams") +
  ggtitle("Egg mass by mating system")
