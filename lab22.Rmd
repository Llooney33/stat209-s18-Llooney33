...
**Answer**:yes they are significant, slow and normal speeds have higher perfectionism scores
...
**Answer**: Some people may have a pressing matter to attend to and have to finish quickly and others might leave the window open and take a break, then return to the test
...
```{r}
model<- lm_basic(social_boldness ~ 1 + apprehension + dominance, data = pf)
reg_table(model, level = .95)
```
...
**Answer**: No I expected apprehension to be negatively correlated with social boldness

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model<- lm_basic(social_boldness ~ 1 + apprehension + dominance + speed + gender + fct_lump(country, 5), data = pf)
reg_table(model, level = .95)
```

Does this change the slopes for your two traits much?

**Answer**: The slopes slightly increased when looking at speed, but otherwise stayed the same.

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**: Social boldness is correlated almost directly with apprehension and dominance, with the highest discrepency only being roughly a point
