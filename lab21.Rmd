...
**Answer**: The numbers are understood as a specific level of trait, not just a number.
...
```{r}
hun <- filter(pf, country == "HU")
model <- lm_basic(social_boldness ~ 1 + gender, data = hun)
reg_table(model, level = 0.95)
```
...
**Answer**: It has a wider range of values for the confidence interval. This is most likely because there are less observations
...
**Answer**: the difference between male and female social boldness is:
            Male: 11.8219 to 12.803
            Female: 10.8811 to 12.226
...
**Answer**:No it is not statisically significant, which says that there is not a problem with social boldness in women versus men, which is a traditional gender stereotype.  
...
**Answer**: Yes, it is negative .1, which really is not a large change in the data. This does not challenge age sterotypes
...
**Answer**: Australia has the highest score for social boldness, and Canada has the lowest
...
**Answer**:As the basebline rises in score, sensitivity rises in score
